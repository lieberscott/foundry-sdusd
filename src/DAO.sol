...
contract SDUSD_DAO is ERC20 {
  ...
  // Changeable variables
  uint256[4] public changeableVariables = [4, 1.5, 50, 67]; // [ethCollateralRatio, degredationThreshold, quorum, passingPercentage]
  ...
  function finalizeProposal(uint256 _proposalNumber) public {
    ...
    check(proposalNumberToData[_proposalNumber].proposalStart + proposalNumberToData[_proposalNumber].durationOfProposal >= block.timestamp, "Proposal is still open");

    // make sure it's reached quorum
    check(proposalNumberToData[_proposalNumber].quorum > (proposalNumberToData[_proposalNumber].votesFor + proposalNumberToData[_proposalNumber].votesAgainst) / totalVotes, "Proposal did not reach quorum");
    ...
    if (proposalNumberToData[_proposalNumber].votesFor / (proposalNumberToData[_proposalNumber].votesFor + proposalNumberToData[_proposalNumber].votesAgainst) > passingPercentage) {
      ...
      check(proposalNumberToData[_proposalNumber].finalizedByCommunity != true, "Proposal has already been finalized");
      ...
      proposalNumberToData[_proposalNumber].finalized = true;
   ...
   */
  function viewProposal(uint256 _proposalNumber) external view {
