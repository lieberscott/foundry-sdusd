...
!developmentChains.includes(network.name)
    ...
    describe("redeemSDUSD", function () {
      ...
      it.only("Correctly redeems for multiple users above and below degredationThreshold as the price of ETH changes", async () => {
        ...
        // assert.equal(ethers.BigNumber.from("10000").mul(sendValue).sub(adamEthSent.toString()).add(adamEthReceivedLocal).sub(adamMintGasCost.toString()).sub(adamRedeemGasCost.toString()).toString(), adamEndingEthBal.toString());
